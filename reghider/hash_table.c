...
VOID HashTableSetContext(PHASH_TABLE Table, PVOID Context)
{
	DEBUG_ENTER_FUNCTION("Table=0x%p; Context=0x%p", Table, Context);
	
	Table->FreeContext = Context;
		 ...
		 if (Bucket != NULL) {
			do {
				...
				if (Table->FreeFunction != NULL)
					Table->FreeFunction(Tmp, Table->FreeContext);
			} while (Bucket != NULL);
		}
	}
...
/// <summary>
/// Inserts a given element to a hash table only in case it is not already
/// present in the table.
/// </summary>
/// <param name="Table">The hash table</param>
/// <param name="Object">The element to insert, represented by address of its HASH_ITEM structure. </param>
/// <param name="Key">The insertion key.</param>
/// <returns>
/// Returns TRUE if the given element has been inserted and FALSE otherwise. The function cannot fail,
/// meaning that the only reason why the element is not inserted, is that it is already present in the table.
/// </returns>
BOOLEAN HashTableInsertIfNotExists(PHASH_TABLE Table, PHASH_ITEM Object, PVOID Key)
{
	KIRQL irql;
	ULONG32 index = 0;
	BOOLEAN ret = FALSE;
	PHASH_ITEM tableItem = NULL;
	DEBUG_ENTER_FUNCTION("Table=0x%p; Object=0x%p; Key=0x%p", Table, Object, Key);
	HASH_TABLE_IRQL_VALIDATE(Table);

	index = Table->HashFunction(Key) % Table->Size;
	HashTableLockExclusive(Table, index, &irql);
	tableItem = Table->Buckets[index];
	while (tableItem != NULL && !Table->CompareFunction(tableItem, Key))
	...
	while (Akt != NULL) {
		Ret = Table->CompareFunction(Akt, Key);
		 if (Ret) {
			if (Prev != NULL)
				Prev->Next = Akt->Next;
			else Table->Buckets[Index] = Akt->Next;
	...
	for (i = 0; i < Table->Size; ++i) {
		...
		if (CallFreeFunction && Table->FreeFunction != NULL) {
			...
			while (akt != NULL) {
				old = akt;
				akt = akt->Next;
				Table->FreeFunction(old, Table->FreeContext);
			}
		}
...
VOID HashTableToList(PHASH_TABLE Table, PLIST_ENTRY ListHead, LONG EntryOffset)
{
	KIRQL irql;
	ULONG i = 0;
	PHASH_ITEM h = NULL;
	PLIST_ENTRY l = NULL;
	DEBUG_ENTER_FUNCTION("Table=0x%p; ListHead=0x%p; EntryOffset=%i", Table, ListHead, EntryOffset);

	for (i = 0; i < Table->Size; ++i) {
		HashTableLockExclusive(Table, i, &irql);
		h = Table->Buckets[i];
		while (h != NULL) {
			l = (PLIST_ENTRY)((PUCHAR)h + EntryOffset);
			InsertTailList(ListHead, l);
			h = h->Next;
			Table->NumberOfItems--;
		}

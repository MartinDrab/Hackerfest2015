...
#define BlockHeaderInitialize(Header, PoolType, NumberOfBytes, Function, Line)	\
	...
	Header->Signature2 = BLOCK_HEADER_SIGNATURE;		\
...
//	  DEBUG_ENTER_FUNCTION("PoolType=%u; NumberOfBytes=%u; Function=%s; Line=%u", PoolType, NumberOfBytes, Function, Line);
	...
	_HeapValidityCheck();
	if (PoolType != PagedPool && PoolType != NonPagedPool) {
		DEBUG_ERROR("Unknown pool type %u", PoolType);
		__debugbreak();
	}

	ret = ExAllocatePoolWithTag(PoolType, wholeSize, (ULONG)'HgbD');
	if (ret != NULL) {
...
//   DEBUG_ENTER_FUNCTION("Address=0x%p", Address);
	...
	tmp = CONTAINING_RECORD(_poolLists[header->PoolType].Flink, DEBUG_BLOCK_HEADER, Entry);
	while (&tmp->Entry != &_poolLists[header->PoolType]) {
		if (tmp == header)
			break;

		tmp = CONTAINING_RECORD(tmp->Entry.Flink, DEBUG_BLOCK_HEADER, Entry);
	}
	
	if (tmp == header) {
		RemoveEntryList(&header->Entry);
		_PoolUnlock(header->PoolType, Irql);
		ExFreePoolWithTag(header, (ULONG)'HgbD');
	} else {
		_PoolUnlock(header->PoolType, Irql);
		DEBUG_ERROR("A memory block (heeader = 0x%p) was not found in the allocator lists!", header);
		__debugbreak();
	}

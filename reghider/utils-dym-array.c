...
VOID DymArrayLock(PUTILS_DYM_ARRAY Array, PKIRQL Irql)
{
	...
	switch (Array->PoolType) {
		case PagedPool:
			...
			ExAcquireResourceExclusiveLite(Array->LockPaged, TRUE);
			break;
		case NonPagedPool:
...
VOID DymArrayUnlock(PUTILS_DYM_ARRAY Array, KIRQL Irql)
{
	...
	switch (Array->PoolType) {
		case PagedPool:
			ExReleaseResourceLite(Array->LockPaged);
...
NTSTATUS DymArrayInsert(PUTILS_DYM_ARRAY DymArray, SIZE_T Index, PVOID Value)
{
	SIZE_T i = 0;
	NTSTATUS status = STATUS_UNSUCCESSFUL;
	DEBUG_ENTER_FUNCTION("DymArray=0x%p; Index=%u; Value=0x%p", DymArray, Index, Value);

#ifdef _DEBUG
	if (Index > DymArray->ValidLength)
		KeBugCheck(0);
#endif 

	status = DymArrayPushBack(DymArray, Value);
	if (NT_SUCCESS(status)) {
		for (i = DymArray->ValidLength - 1; i > Index; --i)
			DymArray->Data[i] = DymArray->Data[i - 1];

		DymArray->Data[Index] = Value;
	}
...
VOID DymArrayInsertNoAlloc(PUTILS_DYM_ARRAY DymArray, SIZE_T Index, PVOID Value)
{
	SIZE_T i = 0;
	DEBUG_ENTER_FUNCTION("DymArray=0x%p; Index=%u; Value=0x%p", DymArray, Index, Value);
	...
	DymArrayPushBackNoAlloc(DymArray, Value);
	for (i = DymArray->ValidLength - 1; i > Index; --i)
		DymArray->Data[i] = DymArray->Data[i - 1];
...
PVOID DymArrayRemove(PUTILS_DYM_ARRAY DymArray, SIZE_T Index)
{
	SIZE_T i = 0;
	PVOID ret = FALSE;
	DEBUG_ENTER_FUNCTION("DymArray=0x%p; Index=%u", DymArray, Index);
	...
	ret = DymArray->Data[Index];
	for (i = Index; i < DymArray->ValidLength - 1; ++i)
		DymArray->Data[i] = DymArray->Data[i + 1];

	--DymArray->ValidLength;
